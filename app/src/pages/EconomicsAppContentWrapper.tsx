...
import {
  economicsScheduleData,
  economicsTotalWeeks,
  economicsSubjectsList,
} from "@/data/economicsScheduleData";
...
export default function EconomicsAppContentWrapper() {
  ...
  const sharedClassData = useMemo(() => {
    ...
    if (classId) {
      return economicsScheduleData.find((c) => c.id === classId) || null;
    }
    return null;
  }, []);
  ...
  const uniqueSubjects = Array.from(
    new Set(economicsScheduleData.map((c) => c.subject)),
  );
    ...
    <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      {showSubjects ? (
        <SubjectsSection
          ...
          subjectsList={economicsSubjectsList}
          scheduleData={economicsScheduleData}
        />
      ) : (
        <>
          <div className="mb-6">
          ...
          {!filters.search &&
            ...
            filters.shift === "all" &&
            filters.day === "all" && (
              <TodaySchedule scheduleData={economicsScheduleData} />
            )}
