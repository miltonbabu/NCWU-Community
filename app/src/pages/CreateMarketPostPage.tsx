...
export default function CreateMarketPostPage() {
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log("Uploading images to market...");
      ...
      if (response.success && Array.isArray(response.urls) && response.urls.length > 0) {
        console.log("Setting images:", response.urls);
        ...
        console.error("response.urls:", response.urls);
        toast.error(response.message || "Upload failed - no URLs returned");
      }
    } catch (error) {
  ...
  const addTag = (inputTag?: string) => {
    ...
    const newTags = rawInput
      .split(',')
      .map(t => t.trim().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]/g, ""))
      .filter(t => t.length > 0);
    ...
    if (duplicateCount > 0) {
      toast.error(`${duplicateCount} tag${duplicateCount > 1 ? 's' : ''} already added`);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await marketApi.createPost({
        title: title.trim(),
