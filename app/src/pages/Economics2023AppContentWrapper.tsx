...
import { economics2023ScheduleData, economics2023TotalWeeks, economics2023SubjectsList } from "@/data/economics2023ScheduleData";
...
export default function Economics2023AppContentWrapper() {
  ...
  const sharedClassData = useMemo(() => {
    ...
    if (classId) return economics2023ScheduleData.find((c) => c.id === classId) || null;
    return null;
  }, []);
  ...
  const filteredClasses = useScheduleFilter(economics2023ScheduleData, filters);
  const isDark = resolvedTheme === "dark";

  const uniqueSubjects = Array.from(new Set(economics2023ScheduleData.map((c) => c.subject)));
  ...
  return (
    <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      {showSubjects ? (
        <SubjectsSection onSubjectClick={handleSubjectClick} subjectsList={economics2023SubjectsList} scheduleData={economics2023ScheduleData} />
      ) : (
        <>
          <div className="mb-6">
            <FilterBar filters={filters} onFilterChange={setFilters} totalWeeks={economics2023TotalWeeks} totalClasses={economics2023ScheduleData.length} filteredCount={filteredClasses.length} />
          </div>
          {!filters.search && filters.week === "all" && filters.shift === "all" && filters.day === "all" && <TodaySchedule scheduleData={economics2023ScheduleData} />}
          <button onClick={() => setShowSubjects(true)} className={`sm:hidden w-full mb-6 p-4 rounded-2xl backdrop-blur-xl ${isDark ? "bg-white/5 border-white/10 text-white" : "bg-slate-50 border-slate-200 text-slate-900"} border flex items-center justify-between`}>
            <div className="flex items-center gap-3"><BookOpen className={`w-5 h-5 ${isDark ? "text-red-300" : "text-red-600"}`} /><span className="font-medium">Browse Subjects</span></div>
