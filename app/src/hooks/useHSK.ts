...
export function useHSKProgress() {
  ...
  const getLevelProgress = useCallback((level: HSKLevel): HSKProgress => {
  ...
  const markWordLearned = useCallback(async (level: HSKLevel, wordId: string, mastered: boolean = false) => {
    const current = getLevelProgress(level);
    const isAlreadyLearned = current.practicingWords.includes(wordId) || current.masteredWords.includes(wordId);

    let newProgress: HSKProgress;
    if (mastered) {
      newProgress = {
        ...current,
        wordsLearned: isAlreadyLearned ? current.wordsLearned : current.wordsLearned + 1,
        lastStudied: new Date().toISOString(),
        masteredWords: [...current.masteredWords.filter(id => id !== wordId), wordId],
        practicingWords: current.practicingWords.filter(id => id !== wordId),
      };
    } else {
  ...
  const updateQuizStats = useCallback(async (level: HSKLevel, score: number) => {
    const current = getLevelProgress(level);
    const newQuizzesTaken = current.quizzesTaken + 1;
    const newAverage = ((current.averageScore * current.quizzesTaken) + score) / newQuizzesTaken;
  ...
  const getOverallProgress = useCallback(() => {
    ...
    const totalWordsLearned = levels.reduce((sum, level) => sum + (progress[level]?.wordsLearned || 0), 0);
    const totalQuizzes = levels.reduce((sum, level) => sum + (progress[level]?.quizzesTaken || 0), 0);
    const avgScores = levels.filter(l => progress[l]?.quizzesTaken > 0).map(l => progress[l].averageScore);
...
export function useHSKQuiz() {
  ...
  const loadQuizResults = useCallback(async () => {
    if (isAuthenticated && user) {
      try {
        ...
        if (response.success && response.data) {
          setQuizResults((response.data as any[]).map((item: any) => ({
            quizId: item.quiz_id,
            score: item.score,
  ...
  const submitQuiz = useCallback(async (): HSKQuizResult | null => {
    ...
    const answerDetails = currentQuiz.questions.map(q => {
      ...
      const correct = userAnswer.toLowerCase() === q.correctAnswer.toLowerCase();
      if (correct) {
        correctAnswers++;
    ...
    if (isAuthenticated && user) {
      ...
      // Save to localStorage for guests
      const storageKey = getUserStorageKey(QUIZ_RESULTS_KEY);
      localStorage.setItem(storageKey, JSON.stringify([...quizResults, result]));
    }

    return result;
  }, [currentQuiz, answers, startTime, quizResults, isAuthenticated, user]);

  const getQuizHistory = useCallback((level?: HSKLevel) => {
    if (level) {
  ...
  return {
    ...
    isLastQuestion: currentQuiz ? currentQuestionIndex === currentQuiz.questions.length - 1 : false,
...
export function useLanguagePartners() {
  ...
  const toggleFavorite = useCallback(async (partnerId: string) => {
  ...
  const isFavorite = useCallback((partnerId: string) => {
...
export function useWordListHistory() {
  ...
  const addWordList = useCallback(async (list: any) => {
    ...
    if (isAuthenticated && user) {
      ...
      // Save to localStorage for guests
      const storageKey = getUserStorageKey(WORD_LIST_HISTORY_KEY);
      localStorage.setItem(storageKey, JSON.stringify([list, ...wordListHistory]));
    }
  }, [wordListHistory, isAuthenticated, user]);

  const removeWordList = useCallback(async (id: string) => {
    ...
    if (isAuthenticated && user) {
      ...
      // Save to localStorage for guests
      const storageKey = getUserStorageKey(WORD_LIST_HISTORY_KEY);
      localStorage.setItem(storageKey, JSON.stringify(wordListHistory.filter(list => list.id !== id)));
    }
  }, [wordListHistory, isAuthenticated, user]);
  ...
  const clearAllData = useCallback(async () => {
    if (isAuthenticated && user) {
      ...
      const keys = [PROGRESS_STORAGE_KEY, QUIZ_RESULTS_KEY, BOOKMARKS_KEY, FAVORITE_PARTNERS_KEY, WORD_LIST_HISTORY_KEY];
      keys.forEach(key => {
